...
int main(int argc, char** argv) {
	...
	if (argc != 4)
	{
		...
		printf("Use -$ <total ants> <iterations> <input file name> \n");
		return 0;
	}
	...
	setupProblemTSP(myrank, coords,  num_coords, ants_per_colony);
	...
	for (int i = 0; i < iterations; ++i)
	{
		...
		colonyRun(num_coords, ants_per_colony);
		...
		if (flag)
		{
			...
			updatePheromones(num_coords, "MESSAGE", false, RHO);
		...
		// update pheromones
		updatePheromones(num_coords, "AS", true, RHO);
	}
